...
PlotData <-
  ...
  mutate(
    ...
    IMAGENUMBER = paste("Replicate", IMAGENUMBER)
  )
...
ggplot(
  ...
  ggsave(
    file = paste(lp.PROTEIN, " MaxInt By_Replictate Density ", DATE_TODAY, ".svg", sep=""),
    width = 4,
    height = 4
  ) +
  ggsave(
    file = paste(lp.PROTEIN, " MaxInt By_Replictate Density ", DATE_TODAY, ".pdf", sep=""),
    width = 4,
    height = 4
  )