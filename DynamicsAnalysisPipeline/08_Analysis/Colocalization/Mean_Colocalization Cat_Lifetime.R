...
TracksSummary <- rbind(TracksSummary, ProteinData)

LIFETIME_CATS <- c("All", sort(unique(ProteinData$LIFETIME_CAT)))

TracksSummary <-
  TracksSummary %>%
  mutate(
    LIFETIME_CAT = factor(LIFETIME_CAT, levels = LIFETIME_CATS)
  )
...
filename = paste(lp.PROTEIN, "Mean_Colocalization Cat_Lifetime By_Cells")
...
filename = paste(lp.PROTEIN, "Mean_Colocalization Cat_Lifetime By_Imgs")
file.remove(paste(filename, ".csv.gz", sep = ""))
...
filename = paste(lp.PROTEIN, "Mean_Colocalization Cat_Lifetime By_Group")
file.remove(paste(filename, ".csv.gz", sep = ""))
