function done = trackmateAnalysisUNIX(ref_trackmatefile, listtrackmatefile, output_dir, channel, maximum_distance, minimum_association_time, minimum_dwell_time, nbins) 
   % try ref_trackmatefile
        format long
        ...
        full_path_output_dir_plots_2D = [ref_path '/' output_dir '/2D_Plots'];
        full_path_output_dir_plots_3D = [ref_path '/' output_dir '/3D_Plots'];
        full_path_output_dir_plots_link_cost = [ref_path '/' output_dir '/Link_cost_plot'];
        ...
        dist_assoc_time_analysis_table_filtered = addvars(dist_assoc_time_analysis_table_filtered, zeros(height(dist_assoc_time_analysis_table_filtered), 1),  'NewVariableNames', {'GROUP'});
        
        group = 1;
        while ~all(dist_assoc_time_analysis_table_filtered.GROUP);
            index = find(dist_assoc_time_analysis_table_filtered.GROUP == 0,1);
            len = height(dist_assoc_time_analysis_table_filtered(ismember(dist_assoc_time_analysis_table_filtered.TRACK_KEY_ref_track_spots_table, dist_assoc_time_analysis_table_filtered.TRACK_KEY_ref_track_spots_table(index)),:));
            dist_assoc_time_analysis_table_filtered(ismember(dist_assoc_time_analysis_table_filtered.TRACK_KEY_ref_track_spots_table, dist_assoc_time_analysis_table_filtered.TRACK_KEY_ref_track_spots_table(index)),:).GROUP = repmat(group, len, 1);
            dist_assoc_time_analysis_table_filtered = trackmateFindColTracks(dist_assoc_time_analysis_table_filtered, group);
            group = group +1;
        end
        ...
        %% plot colocalization
        %plot all tracks 2D
        for group = transpose(unique(dist_assoc_time_analysis_table_filtered.GROUP));
            ref_track_key = dist_assoc_time_analysis_table_filtered(ismember(dist_assoc_time_analysis_table_filtered.GROUP, group),:).TRACK_KEY_ref_track_spots_table;
            if length(ref_track_key)==1;
                ref_track_key = ref_track_key{1};
            end
            ...
            track_key = dist_assoc_time_analysis_table_filtered(ismember(dist_assoc_time_analysis_table_filtered.GROUP, group),:).TRACK_KEY_track_spots_table;
            if length(track_key)==1;
                track_key = track_key{1};
            end
            plot_table = track_spots_table(ismember(track_spots_table.TRACK_KEY, track_key),:);
            ...
            to_plot.set_title('Visualization of Complemented Tracks');
            to_plot.set_names('x', 'X-Position (?m)', 'y', 'Y-Position (?m)', 'color', 'Tracks');
            to_plot.draw();
            to_plot.export('file_name', strcat('Colocalized_tracks_', num2str(group)),'export_path',full_path_output_dir_plots_2D, 'file_type','pdf');
            close all;
        end
        ...
        for group = transpose(unique(dist_assoc_time_analysis_table_filtered.GROUP));
            ...
            to_plot.set_names('x', 'X-Position (?m)', 'y', 'Frame','z', 'Y-Position (?m)', 'color', 'Tracks');
            to_plot.draw();
            %to_plot.export('file_name', ref_track_key{1},'export_path',full_path_output_dir_plots_3D, 'file_type','pdf');
            savefig([full_path_output_dir_plots_3D '/' num2str(group) '.fig']);
            close all;
        end
        ...
        for group = transpose(unique(dist_assoc_time_analysis_table_filtered.GROUP));
            ...
            plot_table = innerjoin(track_spots_table(ismember(track_spots_table.TRACK_KEY, transpose(unique(dist_assoc_time_analysis_table_filtered(ismember(dist_assoc_time_analysis_table_filtered.TRACK_KEY_track_spots_table, track_key),:).TRACK_KEY_track_spots_table))),:), track_table, 'LeftKeys', {'ID', 'TRACK_KEY'}, 'RightKeys', {'SPOT_SOURCE_ID', 'TRACK_KEY'});
            plot_table = sortrows(plot_table,'FRAME');
            figure();
            ...
            to_plot.export('file_name', num2str(group),'export_path',full_path_output_dir_plots_link_cost, 'file_type','pdf');
            close all;
        end
        %% plot analysis 
        ...
        plot_assoc_time.set_title(['Distribution of Time Association for Complemented ' channel(1) ' Tracks']);
        plot_assoc_time.set_names('x', 'Association Time (s)', 'y', 'Frequency (Tracks Count)', 'Color', 'Channel');
        plot_assoc_time.set_text_options('font', 'Arial');
        plot_assoc_time.draw();
        plot_assoc_time.export('file_name', 'Distribution_time_association','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        spot_concentration_plot.set_title('Spot Density by Channel');
        spot_concentration_plot.set_names('x', 'Time (s)', 'y', 'Spots (Count)', 'Row', 'CHN.');
        spot_concentration_plot.set_text_options('font', 'Arial');
        spot_concentration_plot.draw();
        spot_concentration_plot.export('file_name', 'Spot_density','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        spot_concentration_plot.geom_point();
        spot_concentration_plot.set_title('Spot Density by Channel');
        spot_concentration_plot.set_names('x', 'Time (s)', 'y', 'Spots (Count)');
        spot_concentration_plot.set_text_options('font', 'Arial');
        spot_concentration_plot.draw();
        spot_concentration_plot.export('file_name', 'Spots_tracks_concentration','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        spot_location_plot.set_title('Spot Location by Channel');
        spot_location_plot.set_names('x','X-Position (?m)', 'y', 'Y-Position (?m)', 'color','Spots (count)'); %'column','Channel'
        spot_location_plot.set_text_options('font', 'Arial');
        spot_location_plot.draw();
        spot_location_plot.export('file_name', 'Spots_location_tracks','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        spot_location_plot.set_text_options('font', 'Arial');
        spot_location_plot.draw();
        spot_location_plot.export('file_name', 'Spots_location','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        not_related_table = addvars(not_related, repmat({'Independent Molecules'},height(not_related), 1),'before', 'CHANNEL_NUMBER', 'NewVariableNames', 'STATUS');
        ...
        related_table = addvars(related, repmat({'Complemented Molecules'},height(related), 1),'before', 'CHANNEL_NUMBER', 'NewVariableNames', 'STATUS');
        ...
        dwell_time_plot_related_not_related.set_title('Dwell Time of Independent and Complemented Molecules');
        dwell_time_plot_related_not_related.set_names('x', 'Dwell Time (s)', 'y', 'Frequency (Tracks Count)', 'column', 'Channel', 'Color', 'Status');
        dwell_time_plot_related_not_related.set_text_options('font', 'Arial');
        dwell_time_plot_related_not_related.draw();
        dwell_time_plot_related_not_related.export('file_name', 'Dwell_time_independent_complemented','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        plot_total_intensity.set_title('Total Intensity of Independent and Complemented Molecules');
        plot_total_intensity.set_names('x', 'Type', 'y', 'Total Intensity (a.u.)', 'Color', 'Type');
        plot_total_intensity.set_text_options('font', 'Arial');
        plot_total_intensity.draw();
        plot_total_intensity.export('file_name', 'Total_intensity_related_not_related','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        plot_displacement.set_title('Displacement of Tracks');
        plot_displacement.set_names('x', 'Type', 'y', 'Displacement (?m)', 'Color', 'Status');
        plot_displacement.set_text_options('font', 'Arial');
        plot_displacement.draw();
        plot_displacement.export('file_name', 'Displacement_independent_comlemented_molecules','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        to_plot_total_intensity_related_individual.set_text_options('font', 'Arial');
        to_plot_total_intensity_related_individual.draw();
        to_plot_total_intensity_related_individual.export('file_name', 'SNR_related_tracks','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        plot_track_features_mdt.set_text_options('font', 'Arial');
        plot_track_features_mdt.draw();
        plot_track_features_mdt.export('file_name', 'REF_TRACKS_MAX_DISTANCE_TRAVELED','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        plot_track_features_tdt.set_text_options('font', 'Arial');
        plot_track_features_tdt.draw();
        plot_track_features_tdt.export('file_name', 'REF_TRACKS_TOTAL_DISTANCE_TRAVELED','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        plot_track_features_lfp.set_names('x', 'Spots (Count)', 'y', 'Linearity of Forward progression');
        plot_track_features_lfp.set_text_options('font', 'Arial');
        plot_track_features_lfp.draw();
        plot_track_features_lfp.export('file_name', 'REF_TRACKS_LINEARITY_OF_FORWARD_PROGRESSION','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        to_plot.set_names('x', 'X-Position (?m)', 'y', 'Y-Position (?m)', 'Color', 'Dwell Time (s)');
        to_plot.set_text_options('font', 'Arial');
        to_plot.draw();
        to_plot.export('file_name', 'Ref_tracks_dwelltime','export_path',full_path_output_dir_analysis, 'file_type','pdf');
        close all;
        ...
        for trackmatefile = listtrackmatefile;
            ...
            plot_track_features_mdt.set_text_options('font', 'Arial');
            plot_track_features_mdt.draw();
            plot_track_features_mdt.export('file_name', 'TRACKS_MAX_DISTANCE_TRAVELED','export_path',full_path_output_dir_analysis, 'file_type','pdf');
            close all;
            ...
            plot_track_features_tdt.set_text_options('font', 'Arial');
            plot_track_features_tdt.draw();
            plot_track_features_tdt.export('file_name', 'TRACKS_TOTAL_DISTANCE_TRAVELED','export_path',full_path_output_dir_analysis, 'file_type','pdf');
            close all;
            ...
            plot_track_features_lfp.set_text_options('font', 'Arial');
            plot_track_features_lfp.draw();
            plot_track_features_lfp.export('file_name', 'TRACKS_LINEARITY_OF_FORWARD_PROGRESSION','export_path',full_path_output_dir_analysis, 'file_type','pdf');
            close all;
            ...
            to_plot.geom_line();
            to_plot.set_title(strcat('All ' ,' ', channel(2), ' Tracks Colored by Dwell Time'));
            to_plot.set_names('x', 'X-Position (?m)', 'y', 'Y-Position(?m)', 'Color', 'Dwell Time (s)');
            to_plot.set_text_options('font', 'Arial');
            to_plot.draw();
            to_plot.export('file_name', 'tracks_dwell_time','export_path',full_path_output_dir_analysis, 'file_type','pdf');
            close all;
            ...
            to_plot_total_intensity.set_title(['Max Intensity vs. Frame for ' channel(2)]);
            to_plot_total_intensity.set_names('x', 'Time (s)', 'y', 'Max Intensity (a.u.)');
            to_plot_total_intensity.set_text_options('font', 'Arial');
            to_plot_total_intensity.draw();
            to_plot_total_intensity.export('file_name', 'max_intensity','export_path',full_path_output_dir_analysis, 'file_type','pdf');
            close all;
            ...
            plotdwelltime.set_title(['Total Dwell Time ' channel(2)]);
            plotdwelltime.set_names('x', 'Dwell Time (s)', 'y', 'Tracks (Count)');
            plotdwelltime.set_text_options('font', 'Arial');
            plotdwelltime.draw();
            plotdwelltime.export('file_name', 'Totaldwelltime_other','export_path',full_path_output_dir_analysis, 'file_type','pdf');
            close all;
        end
        ...
        try
            ...
            title('Exponetial Fit for Colocalized Molecules');
            set_names('x', 'Dwell Time (s)', 'y', 'Frequency (Tracks Count)');
            legend('off');
            saveas(gcf,[full_path_output_dir_analysis_hl '/exp_fit_colocalized'],'pdf');
            close all

            gg_hist_col = gramm('x', alltracks_col.NUMBER_SPOTS);
            gg_hist_col.stat_bin();
            gg_hist_col.set_title('Distribution of Colocolized Molecules');
            gg_hist_col.set_names('x', 'Dwell Time (s)', 'y', 'Frequency (Tracks Count)');
            gg_hist_col.draw();
            gg_hist_col.export('file_name', 'Dwell_time_dist_col','export_path',full_path_output_dir_analysis_hl, 'file_type','pdf');
            close all;
            ...
            gg_hist_ncol.set_title('Distribution of Independent Molecules');
            gg_hist_ncol.set_names('x', 'Dwell Time (s)', 'y', 'Frequency (Tracks Count)');
            gg_hist_ncol.draw();
            gg_hist_ncol.export('file_name', 'Dwelltime_dist_notcol','export_path',full_path_output_dir_analysis_hl, 'file_type','pdf');
            close all;
            ...
            title('Exponetial Fit for Independent Molecules');
            set_names('x', 'Dwell Time (s)', 'y', 'Frequency (Tracks Count)');
            legend('off');
            saveas(gcf,[full_path_output_dir_analysis_hl '/exp_fit_notcolocalized'],'pdf');
            close all;
