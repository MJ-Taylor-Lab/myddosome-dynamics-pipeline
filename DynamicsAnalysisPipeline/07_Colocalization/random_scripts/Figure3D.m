...
tracks_13 = tracks_13(:,{'TIME_CALIBRATED','CELL_NAME','SYSTEM_NAME'})
tracks_13 = grpstats(tracks_13, {'TIME_CALIBRATED','CELL_NAME', 'SYSTEM_NAME'});

figure(1)
to_plot = gramm('x', round(tracks_13.TIME_CALIBRATED,0), 'y',log(tracks_13.GroupCount+0.1), 'color', tracks_13.SYSTEM_NAME);
to_plot.stat_summary('type', 'sem','geom',{'bar','black_errorbar'}, 'dodge', 1);
to_plot.axe_property('XLim', [0 100], 'YLim', [0 5] );
to_plot.set_line_options('base_size', 0.2)
to_plot.set_names('y', 'FRACTION','x','sec');
to_plot.draw();
to_plot.export('file_name', 'total_dwelltime_13','export_path',path_to_output, 'file_type','svg');
close all;

tracks_16 = tracks(ismember(tracks.SYSTEM_NAME,'16'),:);
tracks_16 = tracks_16(tracks_16.TIME_CALIBRATED<=MAX_DWELLTIME_SEC,:)
tracks_16 = tracks_16(:,{'TIME_CALIBRATED','CELL_NAME','SYSTEM_NAME'})
tracks_16 = grpstats(tracks_16, {'TIME_CALIBRATED','CELL_NAME', 'SYSTEM_NAME'});

figure(1)
to_plot = gramm('x', round(tracks_16.TIME_CALIBRATED,0), 'y',log(tracks_16.GroupCount+0.1), 'color', tracks_16.SYSTEM_NAME);
to_plot.stat_summary('type', 'sem','geom',{'bar','black_errorbar'}, 'dodge', 1);
to_plot.axe_property('XLim', [0 inf],'YLim', [0 6] );
to_plot.set_line_options('base_size', 0.2)
to_plot.set_names('y', 'FRACTION','x','sec');
to_plot.draw();
to_plot.export('file_name', 'total_dwelltime_16','export_path',path_to_output, 'file_type','svg');
close all;

tracks_16 = tracks(ismember(tracks.SYSTEM_NAME,'16'),:);
tracks_16_c = tracks_16(ismember(tracks_16.COLOCALIZATION,'NO'),:);
tracks_16_c = tracks_16_c(tracks_16_c.TIME_CALIBRATED<=MAX_DWELLTIME_SEC,:)
tracks_16_c = tracks_16_c(:,{'TIME_CALIBRATED','CELL_NAME'})
tracks_16_c = grpstats(tracks_16_c, {'TIME_CALIBRATED','CELL_NAME'});

figure(1)
to_plot = gramm('x', round(tracks_16_c.TIME_CALIBRATED,0), 'y',log(tracks_16_c.GroupCount+0.1));
to_plot.stat_summary('type', 'sem','geom',{'bar','black_errorbar'}, 'dodge', 1);
to_plot.axe_property('XLim', [0 inf],'YLim', [0 7] );
to_plot.set_line_options('base_size', 0.2)
to_plot.set_names('y', 'FRACTION','x','sec');
to_plot.draw();
to_plot.export('file_name', 'not_col_dwelltime_16','export_path',path_to_output, 'file_type','svg');
close all;

tracks_16 = tracks(ismember(tracks.SYSTEM_NAME,'16'),:);
tracks_16_c = tracks_16(ismember(tracks_16.COLOCALIZATION,'YES'),:);
tracks_16_c = tracks_16_c(tracks_16_c.TIME_CALIBRATED<=MAX_DWELLTIME_SEC,:)
tracks_16_c = tracks_16_c(:,{'TIME_CALIBRATED','CELL_NAME'})
tracks_16_c = grpstats(tracks_16_c, {'TIME_CALIBRATED','CELL_NAME'});
...
to_plot = gramm('x', round(tracks_16_c.TIME_CALIBRATED,0), 'y',log(tracks_16_c.GroupCount+0.1));
to_plot.stat_summary('type', 'sem','geom',{'bar','black_errorbar'}, 'dodge', 1);
to_plot.axe_property('XLim', [0 inf],'YLim', [0 2] );
to_plot.set_line_options('base_size', 0.2)
to_plot.set_names('y', 'FRACTION','x','sec');
to_plot.draw();
to_plot.export('file_name', 'col_dwelltime_16','export_path',path_to_output, 'file_type','svg');
close all;